...
def improved_mahaton(x, y):    #曼哈顿可能算的不如余弦值效果好
    return sum(map(lambda i, j: abs(i - j), x, y))/sum(map(lambda i, j: abs(i + j), x, y))
...
for i,tt in enumerate(sentences):  #text
    ...
    for j in list(range(i)):
         value = improved_mahaton(tokenized_text[i], tokenized_text[j])
         simlst1.append(value)
    if i != 0:
        minvalue = min(simlst1)
        minvalid = simlst1.index(minvalue)
#    print(i)
#    print(maxvalue)
#    print(maxvalid) #这里是对于小于i的所有值进行遍历，然后返回最相像的值的index，这个和tokenized_text里面的编码是一致的
        listi = [i, minvalue, minvalid]   #第二个元素可以是tokenized_text[i]
        newlst.append(listi)
    print(i)
newlst.insert(0, [0, '第一行工单', 0])
newlst = pd.DataFrame(newlst, columns=["index", "minvalue", "minvalid"])
newlst.to_csv('同诉问题_IM_test.csv')